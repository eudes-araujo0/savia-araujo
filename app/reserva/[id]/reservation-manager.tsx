...
export default function ReservationManager({ token, initialBooking }: { token: string; initialBooking: PublicBooking }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/bookings/availability?date=${encodeURIComponent(date)}&service=${encodeURIComponent(booking.service)}`)
      .then((response) => response.ok ? response.json() : { times: [], unavailable: [] })
      .then((result: { times?: string[]; unavailable?: string[] }) => { if (active) { setTimes(result.times || []); setUnavailable((result.unavailable || []).filter((slot) => !(date === booking.appointmentDate && slot === booking.appointmentTime))); } });
    return () => { active = false; };
