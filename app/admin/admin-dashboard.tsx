...
import type { Booking, BusinessSchedule, Expense, ScheduleBlock } from '../../db/schema';
...
const navigation: { id: View; label: string; icon: typeof LayoutDashboard }[] = [
  ...
  { id: 'horarios', label: 'Dias e horários', icon: Clock3 },
    ...
    <main className="admin-page">
      ...
      <section className="admin-main">
        ...
        {view === 'visao-geral' && <>
          ...
          {!bookings.length && <p className="admin-empty-banner">O painel está conectado ao banco. Faça um agendamento pelo site para ver o fluxo completo aparecer aqui.</p>}
          ...
          <div className="admin-grid">
            ...
            <section className="admin-panel"><div className="panel-head"><div><h2>Pendências recentes</h2><span>Precisam de confirmação</span></div><button onClick={() => setView('clientes-pendentes')}>Ver todas</button></div><PendingClients bookings={pendingBookings.slice(0, 3)} onOpen={setSelectedBookingId} compact /></section>
          </div>
        ...
        {view === 'horarios' && <ScheduleManager schedule={schedule} onSaved={setSchedule} />}
        ...
        {view === 'clientes-pendentes' && <><section className="pending-hero"><div><p className="eyebrow">Central de confirmação</p><h2>{pendingBookings.length ? `${pendingBookings.length} ${pendingBookings.length === 1 ? 'reserva precisa' : 'reservas precisam'} de atenção.` : 'Tudo confirmado por aqui.'}</h2><span>A aprovação acontece automaticamente após a conferência direta na InfinitePay. Fale com a cliente apenas se precisar acompanhar a pendência.</span></div><div><small>Valor aguardado</small><strong>{money(pendingValue)}</strong></div></section><section className="admin-panel admin-view-panel"><div className="panel-head"><div><h2>Fila de pendências</h2><span>Mais recentes primeiro</span></div><strong>{pendingBookings.length}</strong></div><PendingClients bookings={pendingBookings} onOpen={setSelectedBookingId} /></section></>}
      ...
      {selectedBooking && <BookingDrawer booking={selectedBooking} onClose={() => setSelectedBookingId('')} onStatusChange={changeStatus} onEdit={() => setModal('edit')} onReceiveBalance={receiveBalance} />}
      {modal && <OperationModal mode={modal} agendaDate={agendaDate} booking={modal === 'edit' ? selectedBooking : null} services={services} schedule={schedule} busy={operationBusy} onClose={() => setModal('')} onSubmit={submitOperation} />}
    </main>
  );
}
...
function PendingClients({ bookings, onOpen, compact = false }: { bookings: Booking[]; onOpen: (id: string) => void; compact?: boolean }) {
...
function OperationModal({ mode, agendaDate, booking, services, schedule, busy, onClose, onSubmit }: { mode: AdminModal; agendaDate: string; booking: Booking | null; services: BookableService[]; schedule: BusinessSchedule; busy: boolean; onClose: () => void; onSubmit: (event: React.FormEvent<HTMLFormElement>) => void }) {
...
function paymentProviderLabel(provider: string | null) { return provider === 'infinitepay' ? 'InfinitePay' : provider === 'manual' ? 'Registro administrativo' : provider ? 'Registro anterior' : 'Pagamento ainda não iniciado'; }
